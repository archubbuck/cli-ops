...
function shouldDisableProgress() {
    return Boolean(!process.stdout.isTTY || process.env['CI'] || process.env['CONTINUOUS_INTEGRATION']);
}
/**
 * Create a progress bar for long-running operations
 */
function createProgressBar(options) {
    const { total, start = 0, format = '{bar} {percentage}% | ETA: {eta}s | {value}/{total}', hideCursor = true, clearOnComplete = false, enabled = !shouldDisableProgress(), } = options;
...
async function processWithProgress(items, processor, options = {}) {
    const results = [];
    await withProgressBar(items.length, async (bar) => {
