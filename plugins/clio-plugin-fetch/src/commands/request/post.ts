...
import { formatJSON } from '@cli-ops/shared-formatter'
...
export default class RequestPost extends BaseCommand {
  ...
  protected async execute(): Promise<void> {
    ...
    const spinner = createSpinner({ text: 'Sending request...' })
    spinner.start()

    try {
      const response = await client.post(args.url, flags.data, headers)
