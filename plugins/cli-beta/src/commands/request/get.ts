...
import { formatJSON, formatTable } from '@cli-ops/shared-formatter'
...
export default class RequestGet extends BaseCommand {
  ...
  protected async execute(): Promise<void> {
    ...
    const spinner = createSpinner({ text: 'Making request...' })
    spinner.start()

    try {
      const response = await client.get(args.url, headers, flags.cache)
